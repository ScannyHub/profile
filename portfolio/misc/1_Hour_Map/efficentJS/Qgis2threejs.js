...
Q3D.PolygonLayer.prototype.build = function (parent) {
  var materials = this.materials,
	...
	var geometries = [[],[],[],[],[]]; // Hard coded 2D array for building category
	var count = 0; // ?? Verticie count I think, not sure..
	var IsBuildingCategories = false // Flag for using buffer geometires
	  

  if (this.objType == "Extruded") { //Creates geometry and mesh for polygon outlines
    var createSubObject = function (f, polygon, z) {//Creates individual meshes
	...
	// New function for merging geometry and reducing the overall mesh count
		var ModifiedcreateSubObject = function (f, polygon, z) {
			var shape = new THREE.Shape(Q3D.Utils.arrayToVec2Array(polygon[0]));
			for (var i = 1, l = polygon.length; i < l; i++) {
				shape.holes.push(new THREE.Path(Q3D.Utils.arrayToVec2Array(polygon[i])));
			}
			var geom = new THREE.ExtrudeBufferGeometry(shape, {bevelEnabled: false, amount: f.h}); // New use of BufferGeometry
			geom = geom.toNonIndexed(); //Remove Index (?)
			geom.translate(0,0,z) // Apply building elevation to geometry (not mesh)
			count += geom.attributes.position.count; // ?? not sure what this does
			
			//Appending to 2D array, with 2nd arrays for each category of building
			if(f.m==0){
    ...
    var createObject = function (f) {//Abstraction of createSubObject
  ...
  this.f.forEach(function (f, fid) {
	if (f.polygons.length == 1){
			ModifiedcreateSubObject(f, f.polygons[0], f.zs[0]);
  ...
  if (IsBuildingCategories){
	...
	for ( var i = 0, l = geometries.length; i < l; i ++ ) {
		...
		for ( var j = 0, offset = 0, k = geometries[i].length; j < k; j ++ ) {		
			var building = geometries[i][j];
			
			if(i==0){
			bufferGeometry.merge(building, offset)
			}
			else if(i==1){
			bufferGeometry1.merge(building, offset)
			}
			else if(i==2){
			bufferGeometry2.merge(building, offset)
			}
			else if(i==3){
			bufferGeometry3.merge(building, offset)
			}
			else{
				bufferGeometry4.merge(building, offset)
			}
			//bufferGeometry.merge(geometry, offset); // must be non-indexed BufferGeometry
			offset += building.attributes.position.count;
...
Q3D.Utils.createOverlayGeometry = function (triangles, polygons, zFunc) {
  ...
  for (var i = 0, l = polygons.length; i < l; i++) {
    ...
    // triangulate polygon
    var faces = THREE.ShapeUtils.triangulateShape(poly_geom.vertices, holes); // Altered to ShapeUtils
